...
public class OptimisedProgramStrategyAdaptorTest extends TestCase {
	...
	return Factory.BASIC.createRule(new ArrayList<ILiteral>(),
	...
	return new Facts(new RelationFactory());
    }

    private IRelation createRelation() {
	return new RelationFactory().createRelation();
    }
	...
	@Override
	public IRelation evaluateQuery(IQuery query,
			List<IVariable> outputVariables, JsonArray preferences, int k, MergingStrategy strategy)
			throws ProgramNotStratifiedException, RuleUnsafeException,
			EvaluationException {
	...
	@Override
	public IEvaluationStrategy createEvaluator(IFacts facts,
		List<IRule> rules, List<IQuery> queries,
	...
	OptimisedProgramStrategyAdaptor adaptor = new OptimisedProgramStrategyAdaptor(
	...
	assertTrue(optimiser.rulesOutput == factory.mStrategy.mGivenRules); // Same
									    // object
	assertTrue(optimiser.queryOutput == factory.mStrategy.mGivenQuery); // Same
									    // object
	...
	for (int n = 1; n < N; ++n) {
	    Optimiser1 oN_1 = (Optimiser1) configuration.programOptmimisers
		    .get(n - 1);
	    Optimiser1 oN = (Optimiser1) configuration.programOptmimisers
		    .get(n);
	...
	assertTrue(oN.rulesOutput == factory.mStrategy.mGivenRules); // Same
								     // object
	assertTrue(oN.queryOutput == factory.mStrategy.mGivenQuery); // Same
								     // object
    }
}
