...
public class KnowledgeBase implements IKnowledgeBase {
	 ...
	 * @param queries
	 *            The queries of the knowledge-base.
	 * @param configuration
	...
	public KnowledgeBase(Map<IPredicate, IRelation> inputFacts,
			List<IRule> rules, List<IQuery> queries, Configuration configuration)
			throws EvaluationException {
		if (inputFacts == null)
		...
		if (mConfiguration.programOptmimisers.size() > 0)
			...
			mEvaluationStrategy = mConfiguration.evaluationStrategyFactory
					.createEvaluator(facts, rules, queries, configuration);
	}
	...
	public KnowledgeBase(Map<IPredicate, IRelation> inputFacts,
			...
			Configuration configuration,  JsonArray preferences)
			throws EvaluationException {
	...
	public IRelation execute(IQuery query, List<IVariable> variableBindings, int k, MergingStrategy strategy)
			throws EvaluationException, FileNotFoundException {
		if (query == null)
			throw new IllegalArgumentException(
					"KnowledgeBase.execute() - the query is null.");
		...
		IRelation result = mEvaluationStrategy.evaluateQuery(
				RuleManipulator.removeDuplicateLiterals(query),
				variableBindings,preferences,k, strategy);
	...
	/** The queries of the knowledge-base. */
	private final List<IQuery> mQueries;
