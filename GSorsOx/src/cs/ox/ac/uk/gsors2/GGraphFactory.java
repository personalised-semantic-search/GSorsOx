...
public class GGraphFactory {
	...
	public GPreferencesGraph createPreferencesGraph() {
		return new GPreferencesGraph();
	}

	public GPreferencesGraph createPreferencesGraph(Map<User,List<Pair<ITuple, ITuple>> > prefs) {
		GPreferencesGraph pg = new GPreferencesGraph();
		for (User u: prefs.keySet()){
			pg.addPreference(prefs.get(u), u);
		}
		return pg;
	}
