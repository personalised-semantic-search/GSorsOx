...
public class JsonHelper {
	...
	public static Scenario getScenario(String filename) {
		...
		if (matcher.find()) {
			return Scenario.fromInt(Integer.parseInt(matcher.group()));
		}
		return null;
	}

	public static String getIdUser(String filename) {
		Pattern pattern = Pattern.compile("[0-9]+");
		Matcher matcher = pattern.matcher(filename);
		if (matcher.find()) {
	...
	public static Map<User, List<Pair<IPredicate, IPredicate>>> getGPreferences(
			JsonArray array, List<IRule> rules) {
		Iterator<JsonElement> msg = array.iterator();
		Map<User, List<Pair<IPredicate, IPredicate>>> mp = new HashMap<User, List<Pair<IPredicate, IPredicate>>>();
		while (msg.hasNext()) {
			
			JsonObject o = msg.next().getAsJsonObject();
			User u = new User(getNameUser(o), getIdUser(o), gett(o));
			List<Pair<String, String>> conn = getConnections(o);
			Map<String, IPredicate> mapPredicates = new HashMap<String, IPredicate>();
			for (IRule i : rules) {
				for (IPredicate p : i.getPredicates()) {
